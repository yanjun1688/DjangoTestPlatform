...
class EnvironmentChecker:
    ...
    def check_django_project(self):
        ...
        if os.path.exists('manage.py'):
            ...
            self.issues.append("manage.py不存在，请确保在Django项目根目录")
        
        # 检查settings.py
        settings_path = Path('test_platform/settings.py')
        ...
        if self.issues or self.warnings:
            ...
            if self.issues:
                ...
                if any("未安装" in issue for issue in self.issues):
                    self.print_colored("  • 安装依赖: pip install -r requirements.txt", Colors.YELLOW)
            ...
            if self.warnings:
                ...
                if any("虚拟环境" in warning for warning in self.warnings):
                    ...
                    self.print_colored("  python -m venv .venv", Colors.CYAN)
                    if platform.system() == "Windows":
                        self.print_colored("  .venv\\Scripts\\activate", Colors.CYAN)
                    else:
                        self.print_colored("  source .venv/bin/activate", Colors.CYAN)
        else:
    ...
    def run_all_checks(self):
        ...
        self.print_colored("🔍 Django项目环境检查", Colors.BOLD)
        self.print_colored("=" * 60, Colors.BOLD)
