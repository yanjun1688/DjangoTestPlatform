...
class ApiTestResultModelTest(TestCase):
    ...
    def test_api_test_result_properties(self):
        ...
        # 测试通过test_run访问相关属性
        self.assertEqual(result.test_case_name, self.test_case.title)
        self.assertEqual(result.api_name, self.api_definition.name)
        self.assertEqual(result.api_method, self.api_definition.method)
        self.assertEqual(result.api_url, self.api_definition.url)
